...
def main():
    ...
    parser = argparse.ArgumentParser(description="Benchmark all classifiers (with PCA) and report training times.")
    parser.add_argument("--max-per-class", type=int, default=None, help="Optional cap for samples per class.")
    parser.add_argument("--pca-variance", type=float, default=0.95, help="PCA explained-variance target.")
    ...
    # PCA (consistent feature space for all classifiers in this benchmark)
    from sklearn.decomposition import PCA

    X = X.reshape(X.shape[0], -1)
    X = PCA(n_components=args.pca_variance, random_state=42).fit_transform(X)
    print(f"PCA reduced shape: {X.shape}")

    # Optional outlier removal on PCA space (HDBSCAN + KMeans)
    ...
    chosen = {
        ...
        "svm": {"C": 2.575, "kernel": "poly", "gamma": 0.01, "degree": 3, "r": 1},
    ...
    for name, estimator, params in models:
        ...
        row["pca_variance"] = float(args.pca_variance)
        rows.append(row)
    ...
    out_path = os.path.join(results_dir, "benchmark_training_times_pca.csv")
